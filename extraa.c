...
int flag = 0, count = 0, count1 = 0, dt = 0, dd = 0, dx = 0, da = 0,
db = 0, dat1 = 0, dat2 = 0;
float pos1 = 110.0, pos2 = 300.0;
char* ptr1, * ptr2, * ptr3, * ptr4, * ptr5, * ptr6, * ptr7, * ptr8, * ptr9;
int len1, len2, len3, len4, len5, len6, len7;
void one();
float angle, p1 = 107.0, p2 = 300.0, p3 = 175.0, p4 = 200.0, p5 =
175.0, p6 = 200.0, p7 = 100.0, p8 = 115.0, p9 = 185.0, p10 = 99.0, p11
= 210.0, p12 = 280.0;
float p13 = 300.0, p14 = 262.0, p15 = 300.0, p16 = 114.0, p17 = 216.0,
p18 = 99.0;
...
void output2(GLfloat x, GLfloat y, const char* text)
{
    glPushMatrix();
    glTranslatef(x, y, 0);
...
void displayFirstSlide()
{
    ...
    glPushMatrix();
    glScalef(0.3, 0.3, 1);
    output2(-1300, 1300, "BANGALORE INSTITUTE OF TECHNOLOGY");
    glPopMatrix();

    glPushMatrix();
    glScalef(0.24, 0.24, 1);
    output2(-1600, 1100, "COMPUTER GRAPHICS AND LABORATORY(18CSL62)");
    output2(-400, 900, "MINI PROJECT");
    output2(-400, 20, "CLIENT SERVER ARCHITECTURE");
    glPopMatrix();

    glPushMatrix();
    glScalef(0.2, 0.2, 1);
    output2(-2500, -2000, "1BI19CS083 - MACHUPALLI SREEPRAGNA");
...
void displayserver() // this function displays all the design and simulations.
{
   // glColor3f(255,255,0); // bright yellow
    //glPolygonMode(GL_FRONT_AND_BACK,GL_LINE);-for wireframe

//glColor3f(255,255,255); // back to default white
...
void reshapeserver(int w, int h)
{
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    glViewport(0, 0, w, h);
    glMatrixMode(GL_PROJECTION);
...
void displaypeer()
{
    ...
    //float arr[12];
    float sx,sy;
    float l1x,l1y,l2x,l2y,l3x,l3y,l4x,l4y,l5y,l5x,l6y,l6x;
    for(float ii = 0; ii < 6; ii++)
    {
        float theta = 2.0f * 3.1415926f * ii / 6.0;
        ...
        //arr[ii*2+1]=y;
        glPushMatrix();
        glTranslatef(125.0+x,125.0+y,0.0);
        line();
        glPopMatrix();
        ...
        if(ii>0.0)
        {
           ...
           glBegin(GL_LINES);
           glVertex2f(px,py);
           glVertex2f(185.0+x,185.0+y);
           glEnd();

        }
        else
        {
           sx=185.0+x;sy=185.0+y;

        }
        if(ii==2.0)
        {
           l1x=185.0+x;l1y=185.0+y;

         }
         else if(ii==5.0)
         {
            l2x=185.0+x;l2y=185.0+y;

         }
         else if(ii==0.0)
         {
            l5x=185.0+x;l5y=185.0+y;

         }
         else if(ii==3.0)
         {
            l6x=185.0+x;l6y=185.0+y;

         }
         else if(ii==1.0)
         {
            l3x=185.0+x;l3y=185.0+y;
         }

         else if(ii==4.0)
         {
            l4x=185.0+x;l4y=185.0+y;

         }

        px=185.0+x;py=185.0+y;
...
void CS7(int b) // peer to peer
{

    glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT);
    glutDisplayFunc(displaypeer);
    glutPostRedisplay();
...
float a0x=50.0,a0y=200.0,a1x=50.0,a1y=200.0;
float a2x=50.0,a2y=170.0,a3x=50.0,a3y=170.0;
float a4x=80.0,a4y=185.0,a5x=80.0,a5y=185.0;


float a6x=150.0,a6y=300.0,a7x=150.0,a7y=300.0;
float a8x=150.0,a8y=70.0,a9x=150.0,a9y=70.0;
float a10x=150.0,a10y=185.0,a11x=150.0,a11y=185.0;

void refreshattack()
{
    a0x=50.0;a0y=200.0;a1x=50.0;a1y=200.0;
    a2x=50.0;a2y=170.0;a3x=50.0;a3y=170.0;
    a4x=80.0;a4y=185.0;a5x=80.0;a5y=185.0;


 a6x=150.0;a6y=300.0;a7x=150.0;a7y=300.0;
 a8x=150.0;a8y=70.0;a9x=150.0;a9y=70.0;
 a10x=150.0;a10y=185.0;a11x=150.0;a11y=185.0;
}
...
void attack()
{
  ...
  else if(a6x<250.0)
  {
     ...
     glColor3f(1.0,0.0,0.0);
     glBegin(GL_LINES);
     glVertex2f(a10x,a10y);
     glVertex2f(a11x,a11y);
     glEnd();
     glPopMatrix();
...
void displayattack() // this function displays all the design and simulations.
{

    /*draw_text("https//medium.com/@sreepragnamachupalli/an-overview-on-ddos-attacks-2b475c4e273",0.0,0.50);*/
    glPushMatrix();
...
void CS6(int a) // function for menu option 3.
{
    refreshattack();
    glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT);
    glutDisplayFunc(displayattack);
...
//
//    GLint id6 = glutCreateMenu(CS6);
//    glutAddMenuEntry(" Start Attack", 1);
//
//    GLint id7 = glutCreateMenu(CS7);
//    glutAddMenuEntry("Peer to Peer", 1);
//
//    GLint menuId = glutCreateMenu(cs);
...
//    glutAddSubMenu("DDOS Attack", id6);
//    glutAddSubMenu("Peer to Peer Network", id7);
...
void serverScreen_caller(int *window)  //main function.
{
    glutDestroyWindow(*window);
    glutInitDisplayMode(GLUT_DOUBLE | GLUT_RGB | GLUT_DEPTH);
    glutInitWindowSize(1520, 790);
    glutInitWindowPosition(0, 0);
    *window = glutCreateWindow(" Client-Server Architecture by Pragna ");
    Init();

    glutDisplayFunc(displayserver);
    glutReshapeFunc(reshapeserver);
