...
GLfloat colorslite[][3] = { {1.000, 0.980, 0.804}, {0.902, 0.902,
0.980}, {0.867, 0.627, 0.867}, {0.565, 0.933, 0.565} };

GLfloat colorsdeep[][3] = { {0.804, 0.361, 0.361}, {1.000, 0.271,
0.000}, {1.000, 0.271, 0.000}, {0.000, 0.392, 0.000}, {0.000, 1.000,
1.000}, {0.855, 0.647, 0.125} };

void instructionsscreen();
void specialnotescreen_caller();
void specialnote();
...

{
...

    {
...

        {
            ...
            glScaled(1.5 + scalepackets, 1.5 + scalepackets, 1.5 +
scalepackets);
...

{
    ...
    glVertex2f(a,0.525);
    glVertex2f(a+0.05,0.51);
    glVertex2f(a,0.495);
    glEnd();*/

    glLineWidth(3);

    glBegin(GL_LINES); //movement of datapacket
...

{
    ...
    drawSmallText("By M SreePragna", 0.4, -0.8);

    drawSmallText("1BI19CS083-CSE 6B", 0.4, -0.9);
...

{
    ...
    drawSmallText("A Get request is used to retrieve data from a
server.", -0.95, 0.4);

    drawSmallText("A Post request is used to store some data into the
server for future use.", -0.95, 0.3);

    glColor3f(0.32, 0.123, 0.84);

    drawSmallText("Press '1' for Project Screen or press '5' to Quit",
-0.65, -0.8);
...
void specialnote()
{
    ...
    drawText("Why i choose this Project?", -0.35, 0.7);

    glColor3fv(colorsdeep[4]);

    drawSmallText("CNS-Computer Networks and Security is always
special to me ", -0.95, 0.4);

    drawSmallText("The Three months of CNS Class has taught me many
things which completely changed ", -0.95, 0.3);

   drawSmallText("my career path and my perspective towards life", -0.95, 0.2);

    glColor3f(0.32, 0.123, 0.84);

    drawSmallText("THANK YOU", 0.0, -0.8);

    glColor3f(0.0, 1.0, 0.0);



    drawSmallText("It was hard to me when CNS when 5th Sem was over ",
-0.95, 0.1);

    drawSmallText("and i thought that would be the end of CNS from my
life", -0.95, 0.0);



    drawSmallText("But then when i got to know to about CG Project ",
-0.95, -0.1);
    glColor3f(2.0,0.5,1.0);
    drawSmallText(" Friends call me (Client-Server) Thats my nickname
", -0.55, 0.5);

    //drawSmallText("", -0.95, -0.1);
    glColor3f(1.0,1.0,1.0);

    drawSmallText("the only thing that came to my mind is Computer
Networking (Client-Server)", -0.95, -0.2);

    drawSmallText("This Project somehow got CNS Back to my life", -0.95, -0.3);

    drawSmallText("I would never leave a chance that has CNS in it. ",
-0.95, -0.4);

    drawSmallText("", 0.0, -0.3);

    drawSmallText("So,thats the reason why i choose this project", -0.95, -0.5);
    drawSmallText("I would like to thank Dr.Girija Jagannath ma'am for
getting CNS into my life", -0.95, -0.7);
...
void specialnotescreen_caller()
{
  glutDestroyWindow(window);
    ...
    window = glutCreateWindow("Special Note");

    glutDisplayFunc(specialnote);

    glutIdleFunc(specialnote);
...

{
    ...
    drawText("3. Client-Server Communication ", -0.3, 0.0);
    drawText("  Using TCP&UDP Protocols", -0.3, -0.1);

    glColor3f(1, 0.5, 0);

    drawText("4.Special Note", -0.3, -0.2);

    glColor3f(0, 0, 0);

    drawText("5.Quit ", -0.3, -0.0);
...
Status : Acknoledge from Server\n");

            else if (controllineflag1 == 1)

                sprintf(str, "Request Type : Post\n      Current
Status : Client Posting Packets\n");

            else

                sprintf(str, "Request Type : Post\n        Current
Status : Post Control Request from Client\n");
        }
...

        {
            ...
            if (getpacketflag)
                ...
                sprintf(str, "Request Type : Get          Current
Status : Acknoledge from Client\n");

            else if (aflag)

                sprintf(str, "Request Type : Get          Current
Status : Getting Packets from Server\n");

            else

                sprintf(str, "Request Type : Get          Current
Status : Get Control Request from Client\n");
        }
...

        {
            ...
            sprintf(str, "Make a Get(g) Request or a Post(p) Request
to Continue");
        }
...

{
    ...
    sprintf(str, "Packet Speed : %.3lf", (packetspeed <= 50) ?
packetspeed : 0.000);
